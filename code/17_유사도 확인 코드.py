...
COLLECTIONS = [
    ...
    {"name": "legal_cases", "path": r"C:\\law\\chroma_result"},
    {"name": "terms_clauses", "path": r"C:\\law\\chroma_roll"},
]
...
- 설명이 부족하거나 불분명하다면 follow-up 질문을 1문장으로 자연스럽게 생성하세요.
...
def run_chat():
    ...
    while True:
        ...
        if not advice_given:
            combined_query = case_context
            query_embedding = embedding_model.encode([combined_query])[0].tolist()
            threshold = 0.5
            all_docs = []

            print(f"\n🔍 [유사 문서 검색] 쿼리 요약:\n{combined_query.strip()[:300]}")
            print(f"📌 벡터 차원: {len(query_embedding)}\n")
