    ...
    while True:
        ...
        if INITIAL_MODE:
            ...
            if result.strip().upper() == "YES":
                INITIAL_MODE = False
                fixed_case_context = case_context.strip()  # ✅ 고정 저장
        ...
        if advice_given:
            ...
            if needs_rag_response_with_history(query, fixed_case_context, chat_history):
                print("🔍 문서 기반 정밀 검색을 수행합니다... (히스토리 반영)")
                search_query = fixed_case_context + "\n" + query
                document_context = search_documents(search_query, N_RESULTS // 2)
                answer = generate_rag_response_with_history(query, fixed_case_context, document_context, chat_history)
                response_type = "📚 **문서 기반 응답 (히스토리 반영)**"
            else:
                print("💡 일반 상식 기반으로 응답합니다... (히스토리 반영)")
                answer = generate_general_response(query, fixed_case_context)
